...
Exploring the NOAA Storm Database, this report shows that *tornadoes* have
the worst effect on popluation health while hurricanes have the greatest
economic consequences.
Although the NOAA Storm Database contains records from 1950 - 2013, only records
from 1995 onwards are used as the earlier years have fewer events recorded.
...
```{r}
storms.df <- storms.orig[,c("EVTYPE", "BGN_DATE",
                            "FATALITIES", "INJURIES",
...
ggplot(storms.df, aes(x=bgn_year)) +
    ...
    xlab("Year") + ylab("Number Events") +
    theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1)) +
    geom_histogram(binwidth=1, color="black", fill="white")
```

It looks like better records were kept from 1995 onwards, so `storms.df`
will be subsetted accordingly.
```{r}
storms.df <- filter(storms.df, bgn_year >= 1995)
dim(storms.df)
```
...
ggplot(human.top_summary, aes(x=normalizedEVTYPE, y=souls)) +
  ggtitle(paste("Top", top_n, "Injuries and Fatalities per Event Type\n",
                min(storms.df$bgn_year), " - ", max(storms.df$bgn_year))) +
  xlab("Event Type") + ylab("Count") +
  theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1)) +
  geom_point() +
  geom_text(hjust=1, yjust=0, size=4, color="red",
...
and B = billions.
Numeric exponents are treated as base 10.
As we've seen from the unique() output above, there are also rogue EXP units.
I've decided to treat these as K since that's the smallest EXP unit from section 2.7.
```{r}
