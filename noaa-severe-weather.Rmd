...
Create a `storms.human` dataset that plots the number of (non-zero) injured or
fatalities versus the event type.
```{r}
df <- storms.df[storms.df$FATALITIES>0 | storms.df$INJURIES>0, ]
storms.human <- df %>% group_by(normalizedEVTYPE) %>%
    summarise(souls=sum(INJURIES, FATALITIES))
```
Plot
```{r}
index <- which.max(storms.human$souls)
evtype <- storms.human$normalizedEVTYPE[index]
cnt <- storms.human$souls[index]
ggplot(storms.human, aes(x=normalizedEVTYPE, y=souls)) +
