...
This table shows the observations with the highest counts.
```{r}
arrange(human.summary, desc(souls))
```
...
Plot `damage.summary` and notice the event with the worst impact.
```{r}
# Find event with worst impact
index <- which.max(damage.summary$cost)
evtype <- damage.summary$normalizedEVTYPE[index]
cnt <- damage.summary$cost[index]

# Plot
ggplot(damage.summary, aes(x=normalizedEVTYPE, y=cost)) +
  ggtitle("Property and Crop Damage per Event Type") +
  xlab("Event Type") + ylab("Cost") +
  theme_bw() +
  geom_point() +
  geom_text(hjust=1, yjust=0,
            aes(label=paste0(evtype, ", ", cnt),
                x=damage.summary$normalizedEVTYPE[index-2], y=cnt))
```

This table shows the observations with the highest counts.
```{r}
arrange(damage.summary, desc(cost))
```
